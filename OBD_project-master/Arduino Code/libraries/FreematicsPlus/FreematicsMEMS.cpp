...
// the accelerometer biases calculated above must be divided by 8.
  ...
  int32_t accel_bias_value[3] = {0,0,0};
  accel_bias_value[0] = accel_bias_reg[0] >> 1;
  accel_bias_value[1] = accel_bias_reg[1] >> 1;
  accel_bias_value[2] = accel_bias_reg[2] >> 1;
  ...
  accel_bias_value[0] -= (accel_bias[0]/8); // Subtract calculated averaged accelerometer bias scaled to 2048 LSB/g (16 g full scale)
  accel_bias_value[1] -= (accel_bias[1]/8);
  accel_bias_value[2] -= (accel_bias[2]/8);

  data[0] = (accel_bias_value[0] >> 7) & 0xFF;
  data[1] = (accel_bias_value[0] << 1) & 0xFF;
  data[1] = data[1] | mask_bit[0]; // preserve temperature compensation bit when writing back to accelerometer bias registers
  data[2] = (accel_bias_value[1] >> 7) & 0xFF;
  data[3] = (accel_bias_value[1] << 1) & 0xFF;
  data[3] = data[3] | mask_bit[1]; // preserve temperature compensation bit when writing back to accelerometer bias registers
  data[4] = (accel_bias_value[2] >> 7) & 0xFF;
  data[5] = (accel_bias_value[2] << 1) & 0xFF;
...
// bool MPU9250_9DOF::read(float* rotated_acc)
// {
//    float acc[3] = {0.0f};
//    float gyr[3] = {0.0f};
//    float mag[3] = {0.0f};
...
//       rotated_acc[0] = acc[0] - g[0];
//       rotated_acc[1] = acc[1] - g[1];
//       rotated_acc[2] = acc[2] - g[2];
//    }
//    return true;
// }
